...
from rag_system.rag_system import PromptBuilder, RAGSystem


class TestDocumentLoader:
    """Test cases for the DocumentLoader class."""

    def test_initialization(self, settings: Any, test_data_dir: Path) -> None:
    ...
    def test_get_supported_files(
        self, settings: Any, test_data_dir: Path
    ) -> None:
    ...
    def test_load_document_txt(
        self,
        mock_loader_class: Any,
        settings: Any,
        test_data_dir: Path,
        mock_text_loader: Any,
    ) -> None:
    ...
    def test_get_file_type_summary(
        self, settings: Any, test_data_dir: Path
    ) -> None:
...
class TestTextProcessor:
    ...
    def test_initialization(self, settings: Any) -> None:
    ...
    def test_chunk_documents(
        self, text_processor_instance: Any, sample_documents: Any
    ) -> None:
        ...
        assert all(
            doc.metadata["source"] in ["test.txt", "test2.txt", "short.txt"]
            for doc in chunked_docs
        )

    def test_chunk_empty_documents(self, text_processor_instance: Any) -> None:
    ...
    def test_generate_embeddings(
        self, mock_huggingface_embeddings: Any, settings: Any
    ) -> None:
    ...
    def test_generate_embeddings_empty(
        self, text_processor_instance: Any
    ) -> None:
    ...
    def test_process_documents(
        ...
        chunked_docs, embeddings = text_processor_instance.process_documents(
            sample_documents
        )
...
class TestVectorStore:
    ...
    def test_initialization(
        ...
        vector_store = VectorStore(
            settings, embedding_function=mock_embedding_function
        )
    ...
    def test_add_documents(
        ...
        mock_chroma_instance.add_documents.return_value = [
            "doc_0_123",
            "doc_1_456",
            "doc_2_789",
        ]
    ...
    def test_similarity_search(
        self, mock_chroma: Any, settings: Any, mock_embedding_function: Any
    ) -> None:
    ...
    def test_add_documents_mismatch(
        ...
        with pytest.raises(
            ValueError,
            match="Number of documents must match number of embeddings",
        ):
            vector_store = VectorStore(
                settings, embedding_function=mock_embedding_function
            )
...
def test_generate_llm_response_mocked(settings_with_openai_key: Any) -> None:
...
def test_generate_llm_response_raises_on_llm_failure(
    settings_with_openai_key: Any,
) -> None:
...
def test_generate_llm_response_no_api_key(
    settings_without_openai_key: Any,
) -> None:
...
def test_generate_rag_response_with_documents(monkeypatch: Any) -> None:
    ...
    with patch(
        "rag_system.rag_system.RAGSystem.query", return_value=mock_docs
    ):
        with patch(
            "rag_system.rag_system.RAGSystem.generate_llm_response",
...
def test_generate_rag_response_filters_by_score(monkeypatch: Any) -> None:
    """Test that RAG response filters documents by relevance score."""
    # One doc above threshold, one below
    mock_docs = [
        (
            Document(
                page_content="High relevance doc",
                metadata={"source": "doc1.txt"},
            ),
            0.8,
        ),
        (
            Document(
                page_content="Low relevance doc",
                metadata={"source": "doc2.txt"},
            ),
            0.3,
        ),
    ...
    with patch(
        ...
        with patch(
            ...
            # Verify the LLM was called with a prompt containing the high
            # relevance doc
...
def test_generate_rag_response_all_below_threshold(monkeypatch: Any) -> None:
    """Test RAG response when all documents are below relevance threshold."""
    # All docs below threshold
    mock_docs = [
        (
            Document(
                page_content="Low relevance doc 1",
                metadata={"source": "doc1.txt"},
            ),
            0.3,
        ),
        (
            Document(
                page_content="Low relevance doc 2",
                metadata={"source": "doc2.txt"},
            ),
            0.2,
        ),
    ...
    with patch(
        ...
        with patch(
            ...
            assert "Low relevance doc" not in call_args
            assert (
                "I don't know based on the provided information." in response
            )


def test_generate_rag_response_with_custom_prompt(monkeypatch: Any) -> None:
    ...
    with patch(
        ...
        with patch(
            ...
            # Mock the LLM to return a response that includes the custom
            # prompt format
            def mock_llm_response(prompt: str) -> str:
                if (
                    "Custom context:" in prompt
                    and "Relevant document" in prompt
                ):
            ...
            rag_system = RAGSystem(
                settings=Settings(), prompt_builder=CustomPromptBuilder()
            )
            response = rag_system.generate_rag_response("test query")
