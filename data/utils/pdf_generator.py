...
class PDFGenerator:
    ...
    def create_pdf_from_text(
        self, filename: str, content: str, font_size: int = 12
    ) -> str:
        """
    ...
    def create_pdf_from_template(
        ...
        Args:
            filename: Name of the output PDF file
            template: Template string with placeholders ({title}, {content})
    ...
    def create_multiple_pdfs(
        self, documents: List[Tuple[str, str]], font_size: int = 12
    ) -> List[str]:
        """
    ...
    def merge_pdfs(self, input_files: List[str], output_filename: str) -> str:
        ...
        """
        # This is a placeholder - would need pypdf or similar
        # for actual merging. For now, just return the first file as an example
        if input_files:
